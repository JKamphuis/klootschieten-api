 ...
 *
 * Both pages are plain server-rendered HTML, so a regular fetch is enough.
 */
...
function parseTkcDatetime(raw) {
  ...
  // "2025-09-14 10:00:04" (ISO prefix embedded in link text)
...
async function scrapeTkcPage({ url, category }) {
  ...
  // Walk all significant elements in document order
  $('h2, h3, h5, table').each((_, el) => {
    ...
    if (tag === 'h5') {
      // e.g. "Klasse 1" – we prefix with TKC to distinguish from NKB
      currentLeague = text.startsWith('TKC') ? text : `TKC ${text}`;
      return;
    }

    if (tag === 'table') {
      // Re-read current league from nearest preceding h5
      const prevH5 = $(el).prevAll('h5').first().text().trim();
      if (prevH5) currentLeague = prevH5.startsWith('TKC') ? prevH5 : `TKC ${prevH5}`;

      $(el).find('tbody tr').each((_, tr) => {
        const cells = $(tr).find('td');
        if (cells.length < 3) return;

        // col 0 – date (link text contains ISO date + Dutch long form concatenated)
        const dateRaw  = $(cells[0]).text().trim();
        const { date: matchDate, time: matchTime } = parseTkcDatetime(dateRaw);

        // col 1 – "Team A tegen Team B"
        const eventRaw = $(cells[1]).text().trim();
        ...
        // col 2 – score or time
        const scoreRaw = $(cells[2]).text().trim();
        const score    = parseTkcScore(scoreRaw);

        // col 3 – location (optional)
        const locationRaw = cells.length > 3 ? $(cells[3]).text().trim() : null;
...
async function scrapeAllTkc() {
  ...
  for (const page of PAGES) {
    try {
      const matches = await scrapeTkcPage(page);
      all.push(...matches);
    } catch (err) {
